...
<div class="fluid-containter" style="width: 100%">
    <div class="title-con col-md-3">
        <h2 class="title-image" style="">Liked Restaurants</h2>
    </div>
...
//                        $prevAttrIndex=0;
//                    }
                        if ($i == 0 || $blogs[$i]['rest_id'] != $blogs[$i - 1]['rest_id']) {
                            ?>
                            <h3 style="float: left"><?php echo $blogs[$i]['rest_name']; ?> </h3>
                        <?php } ?>
                        <div class="desc-box" style="width:100%; margin-top: 50px;padding: 20px">
                            <div class="desc-inner-box">
                                <p><?php echo $blogs[$i]['description'] ?></p>

                                <p  id="like_text<?php echo $blogs[$i]['blog_restaurants_id']; ?>" class="like " style="float: left;margin-right: 20px;" data-id="<?php echo $blogs[$i]['blog_restaurants_id']; ?>">Unlike</p>  
...
<script>
    ...
    $(".like").on('click', function () {
        ...
        $.ajax({
            ...
            cache: false,
            type: 'post',
            data: {like: 0, restaurant_id: id, userId: <?php echo $user->id ?>},
            datatype: 'json',
